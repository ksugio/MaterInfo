...
from .base import FormView, View, ProjectModel
from .token import GetSessionAccessToken, ParseURL
import os
import requests
import urllib
...
class PullView(View):
        ...
        for item in pull_list:
            if isinstance(item, tuple):
                ...
                if 'updated_at' in data:
                    data.pop('updated_at')
                if method == self.remote_class.UPDATE:
                    lines.append('Update: %s(local %d) < %s' % (model.__class__.__name__, model.id, data))
                elif method == self.remote_class.ADD:
                    lines.append('Add: %s %s' % (cls.model.__name__, data))
                elif method == self.remote_class.DELETE:
                    lines.append('Delete: %s(local %d)' % (model.__class__.__name__, model.id))
            elif isinstance(item, list):
                lines.extend(self.get_list(item))
        return lines
    ...
    def get(self, request, **kwargs):
        ...
        pull_list = self.remote_class().pull_list(model, **option)
        lines = self.get_list(pull_list)
        params = {
            'object': model,
            'lines': lines,
            'brand_name': self.brandName(),
...
class PushView(View):
    ...
    def get_list(self, push_list):
        ...
        for method, model, data, cls in push_list:
            ...
            if 'updated_at' in data:
                data.pop('updated_at')
            if method == self.remote_class.UPDATE:
                lines.append('Update: %s(remote %d) < %s' % (model.__class__.__name__, model.remoteid, data))
            elif method == self.remote_class.ADD:
                lines.append('Add: %s %s' % (model.__class__.__name__, data))
            elif method == self.remote_class.DELETE:
                lines.append('Delete: %s(remote %d)' % (cls.model.__name__, data['id']))
        return lines
    ...
    def get(self, request, **kwargs):
        ...
        push_list = self.remote_class().push_list(model, **option)
        lines = self.get_list(push_list)
        params = {
            'object': model,
            'lines': lines,
            'brand_name': self.brandName(),
...
class Remote():
    ...
    def headers(self, **kwargs):
        return {'Authorization': 'JWT %s' % (kwargs['access'])}
    ...
    def put(self, model, data, files, **kwargs):
        ...
        if url is None:
            return None, kwargs
        response = requests.put(url, headers=self.headers(**kwargs), data=data, files=files)
        if response.status_code == 200:
    ...
    def add(self, model, data, files, **kwargs):
        ...
        if url is None:
            return None, kwargs
        response = requests.post(url, headers=self.headers(**kwargs), data=data, files=files)
...
class FileRemote(Remote):
    ...
    def download(self, name, id, **kwargs):
        ...
        file = requests.get(url, headers=self.headers(**kwargs))
